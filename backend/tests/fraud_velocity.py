...
def _make_org_and_user(db):
    ...
    user = user_crud.create_user(
        db,
        email=f"velocity_{suffix}@example.com",
...
def _base_payload(
    *, user_id: int, organisation_id: int, transactions_last_24h: int
) -> TransactionCreate:
...
def test_velocity_no_match_returns_low_signal_profile(db):
    ...
    payload = _base_payload(
        user_id=user.id, organisation_id=org.id, transactions_last_24h=0
    )
...
def test_velocity_spike_level_1_at_3(db):
    ...
    payload = _base_payload(
        user_id=user.id, organisation_id=org.id, transactions_last_24h=3
    )
    result = scoring_service.score_transaction(db, payload)
...
def test_velocity_spike_level_2_at_5(db):
    ...
    payload = _base_payload(
        user_id=user.id, organisation_id=org.id, transactions_last_24h=5
    )
    result = scoring_service.score_transaction(db, payload)
    ...
    assert {r.rule_code for r in result["matched_rules"]} == {
        "velocity_spike_3",
        "velocity_spike_5",
    }
...
def test_velocity_spike_level_3_at_10_triggers_review(db):
    ...
    payload = _base_payload(
        user_id=user.id, organisation_id=org.id, transactions_last_24h=10
    )
    result = scoring_service.score_transaction(db, payload)
