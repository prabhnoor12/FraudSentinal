...
def test_tenant_isolation_fraud_rules(client):
    ...
    login_a = client.post(
        "/api/v1/auth/login", json={"email": "user_a@orga.com", "password": "StrongPass123!"}
    )
    ...
    login_b = client.post(
        "/api/v1/auth/login", json={"email": "user_b@orgb.com", "password": "StrongPass123!"}
    )
    ...
    response_b = client.get(
        f"/api/v1/fraud-rules/{rule_a_id}", headers={"Authorization": f"Bearer {token_b}"}
    )
    ...
    response_list_b = client.get(
        "/api/v1/fraud-rules/", headers={"Authorization": f"Bearer {token_b}"}
    )
...
def test_tenant_isolation_transactions(client):
    ...
    token_b = client.post(
        "/api/v1/auth/login", json={"email": "tx_b@orgb.com", "password": "StrongPass123!"}
    ).json()["access_token"]
    ...
    response_b = client.get(
        f"/api/v1/transactions/{tx_a_id}", headers={"Authorization": f"Bearer {token_b}"}
    )
    assert response_b.status_code == status.HTTP_404_NOT_FOUND
