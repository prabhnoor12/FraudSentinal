...
def test_enrichment_lookup_ip(client):
    ...
    client.post("/auth/register", json={"email": "geo@test.com", "password": "StrongPass123!", "organisation_name": "Geo"})
    token = client.post("/auth/login", json={"email": "geo@test.com", "password": "StrongPass123!"}).json()["access_token"]
    ...
    with patch("cruds.ip_geolocation_crud.get_geolocation_by_ip", return_value=mock_geo):
        response = client.get(
...
def test_enrichment_lookup_bin(client):
    # Register and login to get token
    client.post("/auth/register", json={"email": "bin@test.com", "password": "StrongPass123!", "organisation_name": "Bin"})
    token = client.post("/auth/login", json={"email": "bin@test.com", "password": "StrongPass123!"}).json()["access_token"]
...
def test_enrichment_signals_integration(client):
    ...
    client.post("/auth/register", json={"email": "sig@test.com", "password": "StrongPass123!", "organisation_name": "Sig"})
    token = client.post("/auth/login", json={"email": "sig@test.com", "password": "StrongPass123!"}).json()["access_token"]
    ...
    with patch("services.enrichment_service._enrich_ip_geolocation") as mock_ip, \
        ...
        mock_ip.return_value = IPGeoSignals(geolocation_available=True, ip_country_code="US")
        mock_card.return_value = BINSignals(bin_available=True, card_brand="Visa", card_type="Credit", issuing_country_code="US", is_prepaid=False, bin_risk_score=10)
